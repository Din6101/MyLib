defmodule MyLibWeb.CredentialsConfirmationLive do
  ...
  def mount(%{"token" => token}, _session, socket) do
    form = to_form(%{"token" => token}, as: "credentials")
  ...
  def handle_event("confirm_account", %{"credentials" => %{"token" => token}}, socket) do
    case Accounts.confirm_credential(token) do
      {:ok, _} ->
        {:noreply,
         socket
